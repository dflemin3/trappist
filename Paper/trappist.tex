...
\begin{deluxetable}{lcc}
\caption{\xxx{Updated Table} Parameter Constraints} \label{tab:constraints}
\tabletypesize{\small}
\tablewidth{0pt}
\tablehead{
...
$f_{sat}$ & $-3.03^{+0.23}_{-0.12}$ & $-3.03^{+0.20}_{-0.10}$,  \\
$t_{sat}$ [Gyr] & $6.64^{+3.53}_{-3.13}$ & $6.62^{+3.56}_{-2.56}$ \\
age [Gyr] & $7.46^{+2.01}_{-2.10}$ & $7.54^{+1.43}_{-1.51}$ \\
...
\section{Validating \approxposterior}
...
\subsection{Synthetic Likelihood Function and Priors}
...
\subsection{MCMC with \emcee} \label{sec:app:trueMCMC}

\xxx{TODO}

\begin{figure*}[h]
\centering
	\includegraphics[width=0.75\textwidth]{../Analysis/Validation/emceeRD5Corner.pdf}
   \caption{\xxx{New Figure: Joint and marginal posterior probability distributions for the 5-dimensional Rosenbrock function likelihood made using \texttt{corner} \citep{ForemanMackey2016}. The black vertical dashed lines on the marginalized distributions indicate the median values and lower and upper uncertainties from the 16th and 84th percentiles, respectively. The blues curves superimposed on the marginal distributions display the adopted prior probability distribution for each parameter. The non-linear form of our likelihood function yields non-Gaussian posterior distributions with banana-shaped correlations between parameters.} }%
    \label{fig:rosenemcee}%
\end{figure*}

\subsection{MCMC with \approxposterior} \label{sec:app:apMCMC}
