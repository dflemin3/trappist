...
\xxx{In Fig.~\ref{fig:convergence}, we display $z_{t,j}$ as a function of iteration for each model parameter for the \approxposterior run presented in the main text. The improvement is not monotonic, however, owing to the stochastic nature of GPs, our hyperparameter optimization scheme, and MCMC sampling that can cause these values to occasionally be slightly worse than previous iterations. }

\begin{figure*}[h]
\centering
	\includegraphics[width=0.75\textwidth]{../Analysis/Convergence/convergence.pdf}
   \caption{\xxx{Absolute difference between the means ($\mu$) of the \approxposterior-derived marginal posterior distributions from the $t^{th}$ and $t-1^{th}$ iterations, scaled by the standard deviations ($\sigma$) of the $t-1^{th}$ iteration's marginal posterior distributions. Note that in \approxposterior, the initial iteration is iteration 0. The black dashed line indicates our adopted convergence threshold of $\epsilon = 0.1$.}}%
