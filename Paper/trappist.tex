...
xxx response to ref changes notes: varied the a sampling parameter within emcee's MCMC to ensure that our derived posteriors are robust. I compared posteriors to prior and found the data to provide constraints beyond prior. Used optimized version of VPLanet and it is still slow. Simulated a population of stars using prior to see what would happen
...
\xxx{Note that our likelihood calculation in principle only requires interpolating the \citet{Baraffe2015} luminosity tracks and evaluating an explicit function of time to compute the XUV luminosity, both computationally-cheap tasks. Each simulation lasts about 10s, however, because \vplanet is a general purpose code designed to simultaneously integrate coupled ordinary differential equations and functions of time to simulate the evolution of an exoplanetary system and its host star \citep[see][ for more details]{Barnes2019}. This generalized structure requires numerous steps to facilitate physical couplings, such as an initial ``verify" step in which physical couplings are validated. Moreover, the \stellar module used in this work simultaneously tracks a star's evolving luminosity, radius, effective temperature, radius of gyration, XUV luminosity, and rotation rate. The additional overhead required by the general design of \vplanet causes the relatively slow runtime.}
...
\centering
   ...
   \caption{Same format as Fig.~\ref{fig:corner}, but derived by \approxposterior. \textbf{The joint and marginal posterior distributions estimated by \approxposterior are in excellent agreement with our \emcee-derived results shown in Fig.~\ref{fig:corner}.}}%
    \label{fig:approx}%
\end{figure*}
...
$f_{sat}$ & $-3.03^{+0.23}_{-0.12}$ & $-3.04^{+0.22}_{-0.12}$,  \\
$t_{sat}$ [Gyr] & $6.64^{+3.53}_{-3.13}$ & $6.77^{+3.55}_{-3.10}$ \\
age [Gyr] & $7.46^{+2.01}_{-2.10}$ & $7.63^{+1.94}_{-1.99}$ \\
$\beta_{XUV}$ & $-1.16^{+0.31}_{-0.30}$ & $-1.16^{+0.29}_{-0.28}$ \\
P$(\mathrm{saturated})$ & $0.40$ & $0.40$ \\
\enddata \vspace*{0.1in}
\tablecomments{Best fit values and uncertainties are derived using the medians, $16^{th}$, and $84^{th}$ percentiles from the marginalized posterior distributions, respectively. \xxx{P$(\mathrm{saturated})$ indicates the posterior probability that TRAPPIST-1 is still in the saturated regime today.} \textbf{Our parameter constraints derived using \approxposterior are in excellent agreement with the values computed using \emcee. We find maximum errors in parameter medians and $1\sigma$ uncertainties of $2.3\%$ and $6.5\%$, respectively.}}
\end{deluxetable}
...
\xxx{TODO: sketch 3 steps of algorithm, citing relevant papers, and note the utility functions to connect GP's learned function to how it improves itself}

\subsection{Training the Gaussian Process} \label{sec:app:gp}

\xxx{TODO}

\subsection{Point Selection} \label{sec:app:point}

\xxx{TODO: figure - training set and points selected by \approxposterior}
